...
def int2word(num):
    ...
    if num < 0:
        ...
        if num % 100 == 0:
            ...
            return DIGIT_TO_WORD[num // 100] + ' hundred and ' + \
                   tens2word(num % 100)
    else:
        magnitude = ((len(str(num)) - 1) // 3) * 3
        left_side = hundreds2word(num // 10 ** magnitude) + " " + \
                    MAGNITUDE[magnitude]
...
if __name__ == '__main__':
    words = (int2word(n) for n in range(1, MAX_NUM + 1))
    num_letters = sum(
            len(word.replace(' ', '').replace('-', '')) for word in words)
    print(num_letters)
